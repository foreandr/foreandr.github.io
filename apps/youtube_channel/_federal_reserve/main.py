...
DATA_SOURCE = "Federal Reserve Economic Data (FRED) https://fred.stlouisfed.org/"
...
def main():
    ...
    print(f"\n{'#'*60}")
    print(f"# FEDERAL RESERVE DATA VISUALIZATION PIPELINE (MODULAR)")
    print(f"# Starting: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'#'*60}\n")
    
    if not os.path.exists(DATA_FILE):
        print(f"Error: {DATA_FILE} not found. Run the FRED scraper first.")
        return
    ...
    # Filter to current year and earlier
